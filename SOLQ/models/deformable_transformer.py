...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None,obj_srcs=None, obj_masks=None, obj_pos_embeds=None,B=None,S=None):
        ...
        if hyp.image_features_self_attend:
            ...
            if hyp.do_self_atn_for_queries:
                ...
                mask_flatten = mask_flatten.view(B,S,E)[:,1:,:].view(B*(S-1),E)
                lvl_pos_embed_flatten = lvl_pos_embed_flatten.view(B,S,E,C)[:,1:,:].view(B*(S-1),E,C)
                valid_ratios = valid_ratios.view(B,S,4,2)[:,1:,:].view(B*(S-1),4,2)
                memory = memory.view(B,S,E,C)
                memory_0 = memory[:,0,:,:].view(B,E,C) # memory frame 0
                memory = memory[:,1:,:,:].view(B*(S-1),E,C) 
        ...
        if hyp.do_deformable_atn_decoder:
            ...
            hs = self.decoder(
                ...
                memory=memory, # memory feature input
                memory_key_padding_mask=mask_flatten, # input mask for memory
                pos=lvl_pos_embed_flatten, # positional embedding for memory
