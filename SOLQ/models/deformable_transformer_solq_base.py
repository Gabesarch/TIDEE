    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        if self.two_stage:
            ...
            if args.do_predict_oop:
                enc_outputs_class2 = self.decoder.class_embed2[self.decoder.num_layers](output_memory)
            enc_outputs_coord_unact = self.decoder.bbox_embed[self.decoder.num_layers](output_memory) + output_proposals
            
            topk = self.two_stage_num_proposals
            # print("averaging scores to get proposals")
            enc_outputs_class_mean = enc_outputs_class[..., 0]
            N = 1
            if args.do_predict_oop:
                enc_outputs_class_mean += enc_outputs_class2[..., 0]
                N += 1
            enc_outputs_class_mean = enc_outputs_class_mean/N
            topk_proposals = torch.topk(enc_outputs_class_mean, topk, dim=1)[1]
            ...
            pos_trans_out = self.pos_trans_norm(self.pos_trans(self.get_proposal_pos_embed(topk_coords_unact, num_pos_feats=int(self.d_model/2))))
        ...
        out = {
            ...
            'seg_memory':seg_memory, 
            'seg_mask':seg_mask
            }
        if self.two_stage:
            if args.do_predict_oop:
                out['enc_outputs_class2'] = enc_outputs_class2
        return out
