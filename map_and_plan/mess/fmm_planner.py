...
class FMMPlanner():
    def __init__(self, traversible, num_rots, step_size, obstructed_actions):
    ...
    def _virtual_steps(self, u_list, state, check_collision=True):
        ...
        for i in range(len(u_list)):
            ...
            if action == 3:
                ...
                angl = t
                x_new = x-np.sin(angl)*self.du
                y_new = y+np.cos(angl)*self.du
                t_new = angl
            elif action == 17:
                angl = t
                x_new = x+np.sin(angl)*self.du
            ...
            if inside:
                ...
                if (action in [3, 15, 16, 17]) and check_collision:
                    for s in np.linspace(0, 1, self.du+5):
                        _x = x * s + (1-s) * x_new
                        _y = y * s + (1-s) * y_new
