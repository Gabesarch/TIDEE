...
from rearrangement.rearrange.constants import STARTER_DATA_DIR, STEP_SIZE
from rearrangement.rearrange.environment import (
    RearrangeTHOREnvironment,
    RearrangeTaskSpec,
)
from rearrangement.rearrange.expert import (
    ...
    def __next__(self) -> RearrangeTaskSpec:
        if len(self.task_spec_dicts_for_current_scene) == 0:
            ...
            if self.shuffle:
                ...
                # print(self.task_spec_dicts_for_current_scene)
...
class RearrangeTaskSampler(TaskSampler):
    def __init__(
        ...
        if shuffle:
            shuffle_ = True
            print("SHUFFLING MAPS")
        else:
            shuffle_ = epochs == float("inf")
