...
from .deformable_transformer_mvideo import build_deforamble_transformer
...
class SOLQ(nn.Module):
    ...
    def forward(self, samples, queries=None, targets=None, init_queries=False):
            ...
            for l in range(self.num_feature_levels):
                ...
                for b in range(B):
                    ...
                    if len(obj_pos_l_)>0:
                        ...
                        obj_pos_l_ = torch.tensor(obj_pos_l_).cuda()
        ...
        hs, batch_inds, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact = self.transformer(
                                                                                                        srcs, 
                                                                                                        masks, 
                                                                                                        poss, 
                                                                                                        query_embeds, 
        ...
        # we need to reorder features so that they follow: batch 0 view 1, batch 0 view 2, ... , batch b view 1, batch b view 2, ... - currently they are ordered: batch 0 view 1, batch 1 view 1, ... , batch 0 view s, batch 1 view s, ... 
        L, _, N, E = hs.shape
        hs = hs.reshape(L, (S-1), B, N, E).permute(0, 2, 1, 3, 4).reshape(L, (S-1)*B, N, E)
        batch_inds = batch_inds.reshape((S-1), B).permute(1, 0).reshape((S-1)*B) # batch inds should now follow the correct ordering
        # print(batch_inds)
        ...
        out["batch_inds"] = batch_inds
        return out, hs[-1].detach()
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, do_masks=True):
        ...
        """
        B, S = hyp.batch_size, hyp.S
        out_logits, out_bbox, out_vector, out_batches = outputs['pred_logits'], outputs['pred_boxes'], outputs['pred_vectors'], outputs['batch_inds']
